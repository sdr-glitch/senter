...
### 완료됨 (기록)
- ~~3D AI 오피스 (office3d.html)~~ → 인플루언서 프롬프트(the-delegation 아이디어)를 센터에 이식. **원본은 `~/.claude` 로컬 설정을 읽지만 브라우저 앱은 그 폴더에 접근 불가** → 재료를 센터의 실제 데이터로 교체(직원 19명·5부서·자료실·업무·보고서·링크함). 구현: `saveOfficeSnapshot()`(app.js, renderAll마다)이 조직·활동 스냅샷을 남기고, 독립 페이지가 스냅샷+IndexedDB를 **읽기만** 함 → 기존 앱·데이터 무변경, LLM 호출 0(비용 0), Three.js는 `vendor/three.min.js`(r150 UMD, file:// 호환). 규칙 준수: 자료=워크스테이션(사람 아님)·진짜 에이전트만 사람·층=부서(인원순, 최상층 경영층, 지상층 로비)·색=부서/발광=업무량(축 분리)·근거 없는 관계 미표시·회의 참석자는 회의실로 옮기고 원자리엔 「회의 중」 표식. **WebGL 불가 대응**: 2D 평면도 대체 뷰를 한 번 만들었다가 **사용자가 "3D로 보고 싶다"며 제거 요청** → 걷어내고, 대신 `showGpuHelp()`가 원인별 해결 순서(크롬 `chrome://settings/system` 가속 켜기 → `chrome://flags/#ignore-gpu-blocklist` Enabled → 엣지 경로 → `chrome://gpu`로 확인 → 폰으로 열기)를 단계별로 안내. `probeGL()`은 webgl2/webgl/experimental × 5옵션 × (기존·새 캔버스)로 재시도해 약한 GPU도 최대한 3D로 살림. **교훈: 사용자가 원하는 건 대체물이 아니라 원래 기능 — 폴백보다 '켜는 방법'을 정확히 알려주는 게 답일 수 있다.** **실제로 밟은 지뢰 3개: ① 서비스 코어를 지상층 깊이 기준으로 배치해 상층 유리를 관통(→ 전 층 최대 깊이 기준) ② 기본 카메라가 건물 뒤를 봐서 사인·조각상·입구가 안 보임(→ 정면 +z 시점) ③ WebGL은 렌더 직후 버퍼를 비워 나중 readPixels가 균일값 반환(→ `sample()` 훅이 render 직후 같은 프레임에서 읽음)**. 투어 mp4는 사용자 요청으로 보류.
  - **후속 사고와 수정(사용자 환경에서 `Error creating WebGL context` 발생)**: ① 오류를 "데이터 문제"로 잘못 안내 → 원인별 분기 ② WebGL 컨텍스트 단일 시도 → `probeGL()`이 webgl2/webgl/experimental × 4가지 옵션 조합 재시도 ③ **THREE 의존 객체를 최상위에서 즉시 생성**(`M` 재료, `world = new THREE.Group()`, `Raycaster`)해 라이브러리 없으면 스크립트 전체가 죽어 평면도도 못 그림 → 전부 지연 생성 ④ **WebGL로 쓰인 캔버스는 `getContext("2d")`가 null** → 평면도 전환 시 캔버스 요소 교체. 검증: `office2d.mjs` 9시나리오(데스크톱·모바일 WebGL 차단, three.js 차단, GPU 정상 3D, 강제 전환, 데이터 없음, 1x1 창, 스크롤, 클릭) 전부 통과.
