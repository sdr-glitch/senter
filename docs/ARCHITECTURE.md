...
2. **회의록 열람 UI** — meetings에 10개 저장되지만 지난 회의록을 보는 화면이 없음
3. **업무 보드 필터/검색** — 완료 업무가 수십 건 쌓이면 필요
4. **Puter 모델 고정 옵션** — 현재 서비스 기본 모델 사용. 품질 이슈 시 모델 지정 검토
5. **스튜디오 ↔ 센터 데이터 연결** — 스튜디오 본부의 SNS 팔로워 수치를 센터 보고서에 포함
6. **iOS 홈화면 PWA** — manifest.json + 아이콘이면 앱처럼 설치 가능
7. **로드맵 커스터마이즈** — 현재 리빙 계정 고정. 주제 바꾸면 로드맵도 바뀌게
